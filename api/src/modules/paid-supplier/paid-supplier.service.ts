...
export class PaidSupplierService {
  ...
  async create(createPaidSupplierDto: CreatePaidSupplierDto) {
    const payment = await this.prisma.payment.findUnique({
      where: { id: createPaidSupplierDto.paymentId },
    });
    if (payment) {
      throw new HttpError({ message: 'Payment Not Found', code: 404 });
    }
  ...
  async findAll(dto: FindAllPaidSupplierQueryDto) {
    ...
    const where: Prisma.PaidSupplierWhereInput = {
      supplierId,
      paidDate: { gt: minPaidDate, lt: maxPaidDate },
      isDeleted: false,
    };
